# Initialization
using Plots, LazyGrids, Printf, LinearAlgebra, LoopVectorization
...
# --------------------------------------------------------------------
# Initial strain -----------------------------------------------------
γc  .=  γ0 .* exp.( .- (xc2.^2 .+ (yc2.-0.4).^2) ./ ri .^ 2 );
# ================================================================== #
anim = Animation()
# Start time loop ====================================================
for it = 1:nt
    ...
    Δts     =   0.5 * min(Δx,Δy) / max(maximum(Vxc),maximum(Vyc));
    if Δt > Δts 
        @printf("WARNING! Time step is larger than the 
                    advection criterion!\n")
    end
    # Elastic viscosity ----------------------------------------------
    ηel     =   G*Δt
...
# Plot ===============================================================
if success==true    
    gif(anim, "2D_vep_sdw2.gif", fps = 15)
    p1 = heatmap(xc,yc,Vc',title="V")
    p2 = heatmap(xc,yc,P[2:end-1,:]',title="P")    
    p3 = heatmap(xc,yc,γc',title="γc")
    p4 = heatmap(xc,yc,log10.(ηc'),title="log10(ηc)")
