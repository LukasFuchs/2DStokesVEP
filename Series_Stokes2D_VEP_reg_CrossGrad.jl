...
for it = 1:nt
    ...
    @. E_pl[2:end-1,2:end-1] = Δt * εIIpl
    @. E_gradx = (E_pl[3:end,:] - 2.0 * E_pl[2:end-1,:] + E_pl[1:end-2,:]) / dx^2.0
    @. E_grady = (E_pl[:,3:end] - 2.0 * E_pl[:,2:end-1] + E_pl,[:,1:end-2]) / dy^2.0
