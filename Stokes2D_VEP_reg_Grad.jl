...
η_reg       =   0.004       # Regularization viscosity
c_grad      =   1.0 ; #0*1.0e-11     # Gradient factor
# Elastic constants --------------------------------------------------
...
E_pl    =   zeros(ncx+2,ncy+2)      # Plastic strain; centroids
E_grad  =   zeros(ncx,ncy)          # Plastic strain gradient; centroids
E_gradx =   zeros(ncx,ncy)          # Plastic strain gradient; centroids
E_grady =   zeros(ncx,ncy)          # Plastic strain gradient; centroids
...
for it = 1:nt
    ...
    @printf("Step %05d --- Δt: %2.2e, Δx: %2.2e, Δy: %2.2e\n",it,Δt,Δx,Δy)
    # Elastic viscosity ----------------------------------------------
    ηel     =   G*Δt
    # Time -----------------------------------------------------------
    t           += Δt
    ...
    # Plastic strain -------------------------------------------------
    c_grad = c_grad / ( (xmax-xmin)^2 * G * Δt * abs(εbg))
    @printf("Step %05d --- cgrad: %2.2e\n\n", it, c_grad)
    @. E_pl[2:end-1,2:end-1] = Δt * εIIpl + E_pl[2:end-1,2:end-1]
    @. E_pl[1,:] = E_pl[2,:]; E_pl[end,:] = E_pl[end-1,:]; 
    @. E_pl[:,1] = E_pl[:,2]; E_pl[:,end] = E_pl[:,end-1];
    @. E_gradx = (E_pl[3:end,2:end-1] - 2.0 * E_pl[2:end-1,2:end-1] + E_pl[1:end-2,2:end-1]) / Δx^2.0
    @. E_grady = (E_pl[2:end-1,3:end] - 2.0 * E_pl[2:end-1,2:end-1] + E_pl[2:end-1,1:end-2]) / Δy^2.0
    @. E_grad = c_grad * (E_gradx + E_grady)
    # Elasticity =====================================================
    ...
    if do_save
        ...
        p2 = heatmap(xc,yc,E_pl[2:end-1,2:end-1]',title="E_pl")
        p3 = heatmap(xc,yc,γc',title="γc")
        #p4 = heatmap(xc,yc,log10.(ηc'),title="log10(ηc)")
        p4 = heatmap(xc,yc,E_grad',title="E_grad")
...
if success==true
    ...
    #p1 = heatmap(xc,yc,Vc',title="V")
    p1 = heatmap(xc,yc,E_grady',title="E_grady")
    #p2 = heatmap(xc,yc,P[2:end-1,:]',title="P")
    p2 = heatmap(xc,yc,E_pl[2:end-1,2:end-1]',title="E_pl")
    #p3 = heatmap(xc,yc,γc',title="γc")
    p3 = heatmap(xc,yc,E_gradx',title="E_gradx")
    #p4 = heatmap(xc,yc,log10.(ηc'),title="log10(ηc)")
    p4 = heatmap(xc,yc,E_grad',title="E_grad")
